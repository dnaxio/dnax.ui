...
| Situation | Behaviour |
| --- | --- |
| Enough room below (≥ `96px`) | Opens **below** the field, gap `offset` (default `8px`) measured from the **control** bottom. |
| Not enough room below, more room above | Flips **above** the field (the entry animation comes from the bottom, via `q-select__popup--up`). |
| The roomier side is still tight | The gap shrinks down to `0px` so the list keeps the space. |
| Any case | `max-height` is capped to the room actually available (never more than the historical `240px`), so the popup never overflows the window edge — the list scrolls inside. |
...
::prose-show-case
<dnax-demo-select demo="offset"></dnax-demo-select>
...
| Value | Effect |
| --- | --- |
| `auto` _(default)_ | Below when there is room (≥ `96px`), flips above otherwise. |
| `bottom` / `top` | Always below / above — no flip. |
| `bottom-start` / `top-start` | Same, anchored on the field's **left** edge. |
| `bottom-end` / `top-end` | Same, anchored on the field's **right** edge. |
...
::prose-show-case
<dnax-demo-select demo="direction"></dnax-demo-select>
